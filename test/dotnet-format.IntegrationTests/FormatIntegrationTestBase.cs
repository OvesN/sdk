...
public abstract class FormatIntegrationTestBase : SdkTest
{
    ...
    /// Runs from the solution's parent directory to match the old script's Set-Location behavior
    /// and resolve nuget.config relative to the solution directory.
    ...
    private void RestoreWithParentSdk(string solutionPath)
    {
        ...
        var nugetConfig = Path.Combine(solutionDir, "nuget.config");
        ...
        RunProcess(ParentDotNetPath, args, ignoreThrowingOnError: true, workingDirectory: solutionDir);
    }
